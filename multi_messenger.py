...
DATA = {
    ...
    "Effelsberg/Parkes": ["data/spass_dr1_1902_healpix_Tb.i.fits", "data/EFFELSBERG_11cm_1_2048.fits"],
    "Dame et al.": "data/lambda_wco_dht2001.fits",
    "IRIS": ["data/IRIS_nohole_4_2048_v2.fits", "data/IRIS_nohole_3_2048_v2.fits", "data/IRIS_nohole_1_1024_v2.fits"],
    "GLIMPSE": "data/GLIMPSE_irac2_4arcm_1_2048_partial.fits",
    "DIRBE": ["data/DIRBE_3_256.fits", "data/DIRBE_2_256.fits", "data/DIRBE_1_256.fits"],
...
def plot_skymaps():
    ...
    axs[1].imshow(get_map_view(DATA["LAB HI Survey"]), vmin=1e-2, vmax=8, norm=None, **OPTS)

    axs[2].imshow(get_map_view(DATA["Effelsberg/Parkes"][0]), norm=LogNorm(0.1,1e1), **OPTS)
    axs[2].imshow(get_map_view(DATA["Effelsberg/Parkes"][1]), norm=LogNorm(1,1e4), **OPTS)
    axs[2].fill_between(np.linspace(*axs[2].get_xlim(), 1000), -0.5, 10.5, color=FACECOLOR)
    axs[2].fill_between(np.linspace(*axs[2].get_xlim(), 1000), 32.5, 43.5, color=FACECOLOR)

    axs[3].imshow(get_map_view(DATA["Dame et al."]), norm=LogNorm(0.8, 1e2), **OPTS)

    iris = get_map_view(DATA["IRIS"][0]) + get_map_view(DATA["IRIS"][1]) + get_map_view(DATA["IRIS"][2])
    axs[4].imshow(iris, norm=LogNorm(2, 1e3), **OPTS)

    dirbe = get_map_view(DATA["DIRBE"][0]) + get_map_view(DATA["DIRBE"][1]) + get_map_view(DATA["DIRBE"][2])
    axs[5].imshow(dirbe, norm=LogNorm(0.5,5e1), **OPTS)

    axs[6].imshow(get_map_view(DATA["WHAM, VTSS and SHASSA"]), norm=LogNorm(1, 1e3), **OPTS)
    ...
    for ax, ttl in zip(axs, title):
        if DESC:
            ax.text(.01, .9, ttl, color="white", ha='left', va='top', transform=ax.transAxes)
        ...
        if TICKS:
            ax.set(
                ...
                xticklabels=["180°", "150°", "120°", "90°", "60°", "30°", "0°", "30°", "60°", "90°", "120°", "150°", "180°"],
                yticklabels=["-15°", "0°", "15°"],
            )
            ax.grid(False)
        else:
